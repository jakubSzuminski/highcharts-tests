...
3. Write a test with assertion that will break the code, then change the Qunit test (not the assertion itself) so it will pass even though assertion returns false. Then edit it so it will fail, but not break the code.
...
3. As a first test (alphabetically), create a test that will break Karma, then run karma again. Then change it (not the assertion) so it will fail instead of break.
